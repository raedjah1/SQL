...
FROM (
    SELECT 
        ...
        CASE 
            -- Validation: If CreditReplacement has an actual numeric value > 0, it's CLOSED (highest priority)
            -- Safely handles varchar values like 'CREDIT' and comma-separated numbers like '2,147.22'
            WHEN rma.[Credit/Replacement] IS NOT NULL 
                 AND ISNUMERIC(rma.[Credit/Replacement]) = 1 
                 AND CASE 
                     -- Only attempt CAST if ISNUMERIC = 1 (prevents errors on varchar values)
                     -- Remove commas (thousand separators) before casting to handle values like '2,147.22'
                     WHEN ISNUMERIC(rma.[Credit/Replacement]) = 1 
                     THEN CAST(REPLACE(rma.[Credit/Replacement], ',', '') AS DECIMAL(18,2))
                     ELSE NULL  -- Return NULL for non-numeric values (skips them)
                     END > 0 THEN 'CLOSED'
            ...
            -- Validation: If Approval Date exists and oem_action is NOT DECLINED and NOT HOLD, it's APPROVED
            WHEN rma.[rma_num_timestamp] IS NOT NULL 
                 AND rma.[oem_action] != 'DECLINED' 
                 AND rma.[oem_action] NOT LIKE 'HOLD%'
    ...
    WHERE rma.[part_code] IS NOT NULL
      -- Filter out records where Supplier will be blank (check source fields directly for better performance)
      AND (
          rma.[manufacturer] IS NOT NULL 
          AND LTRIM(RTRIM(rma.[manufacturer])) != ''
          OR EXISTS (
              SELECT 1
              FROM Plus.pls.PartNoAttribute pna
              INNER JOIN Plus.pls.CodeAttribute ca ON ca.ID = pna.AttributeID
