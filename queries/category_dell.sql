-- Operator Efficiency Dashboard Query - Complete Performance Metrics
-- Uses accurate base tables (Plus.pls.PartTransaction + CodePartTransaction) for precise categorization
-- Replaces the large query that used pls.vPartTransaction view
...
SELECT 
  ...
  CASE WHEN ProcessCategory = 'OVERALL_EFFICIENCY' THEN EfficiencyPercentage ELSE NULL END AS OverallEfficiencyPercentage
FROM (
  SELECT 
    cm.Username,
    cm.Hour,
    ...
    ISNULL(pt.Green_Threshold, 25) as Green_Threshold,  -- Default threshold for uncategorized transactions
    ISNULL(pt.Yellow_Threshold, 12) as Yellow_Threshold,  -- Default threshold for uncategorized transactions
    CASE 
      WHEN (CASE 
              WHEN DATEDIFF(MINUTE, MIN(cm.CreateDate), MAX(cm.CreateDate)) > 0 
              THEN CAST(COUNT(*) AS FLOAT) / DATEDIFF(MINUTE, MIN(cm.CreateDate), MAX(cm.CreateDate)) * 60 
...
FROM (
    SELECT 
      ...
      CASE 
        ...
        ELSE 'Historical'
      END AS IsToday,
      -- COMPLETE CATEGORIZATION using accurate cpt.Description (ID-based lookup)
      CASE
  ...
  LEFT JOIN (
    SELECT 
      pc.ProcessCategory,
      CASE 
        -- Most specific patterns first (categories with multiple keywords)
...
SELECT 
  ...
  PerformanceStatus,
  EfficiencyPercentage,
  NULL AS OverallEfficiencyPercentage
FROM (
  SELECT 
    cm.Username,
    cm.Hour,
...
-- Overall Efficiency per Username (aggregated across all dates/hours/departments)
SELECT 
  Username,
  NULL as Date,
  NULL as Hour,
  'All' as Department,
  'All' as IsToday,
  'OVERALL_EFFICIENCY' as ProcessCategory,
  TransactionCount,
  MinutesWorked,
  TransactionsPerHour,
  Green_Threshold,
  ...
  PerformanceStatus,
  EfficiencyPercentage,
  EfficiencyPercentage AS OverallEfficiencyPercentage
FROM (
  SELECT 
    cm.Username,
    COUNT(*) as TransactionCount,
  ...
  HAVING COUNT(*) <= 2500  -- Higher limit for overall aggregation
) OverallEfficiency
