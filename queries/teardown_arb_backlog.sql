-- Teardown ARB Backlog Query - OPTIMIZED VERSION
-- Uses PartSerial directly (more efficient than PartTransaction)  
-- Checks for child parts to determine IN Process vs Backlog status
...
-- LOB from Current Serial (much simpler and faster)
...
WHERE ps.ProgramID = 10053
    AND ps.SerialNo NOT LIKE 'ARC%'
    -- Optional: Filter for 7-character serials and PalletBoxNo = '0' (from second query)
    -- AND LEN(ps.SerialNo) = 7
    -- AND ps.PalletBoxNo = '0'
