-- ============================================================================
-- GCF BIZTALK INVESTIGATION FOR 11/10/2025
-- ============================================================================
-- Shows all GCF errors from BizTalk for 11/10/2025 with categorization
-- This helps understand what messages exist for Quincy fix attempts on this date
-- ============================================================================

-- Detail: All GCF messages for 11/10/2025
SELECT TOP 100
...
WHERE obm.Source = 'Plus'
  ...
  AND obm.Processed = 'F'
  AND CAST(obm.Insert_Date AS DATE) = '2025-11-10'
...
-- ============================================================================
-- Check: How many "Msg Sent Ok" messages exist for serial numbers that had Quincy fix attempts on 11/10/2025?
-- Note: "Msg Sent Ok" messages typically appear AFTER the fix attempt date
-- ============================================================================
SELECT 
    CAST(obm.Insert_Date AS DATE) AS [MsgSentOkDate],
    obm.Customer_order_No AS [WorkOrder],
    obm.Insert_Date AS [MsgSentOkTimestamp],
    r.createDate AS [QuincyAttemptDate],
...
WHERE obm.Source = 'Plus'
  AND obm.Contract = '10053'
  AND obm.Message_Type IN ('DellARB-GCF_V4', 'DellARB-GCF_V3')
  -- Don't filter by Processed for "Msg Sent Ok" - they are Processed = 'T'
  AND LOWER(obm.Message) LIKE '%msg sent ok%'
  AND obm.Insert_Date > r.createDate  -- After Quincy attempt time (matching fix attempt summary logic)
ORDER BY obm.Insert_Date DESC;

-- Summary count
SELECT 
    'Msg Sent Ok Messages for 11/10/2025 Quincy Fix Attempts' AS [Check],
    COUNT(*) AS [Count],
...
WHERE obm.Source = 'Plus'
  ...
  -- Don't filter by Processed for "Msg Sent Ok" - they are Processed = 'T'
  AND LOWER(obm.Message) LIKE '%msg sent ok%'
  AND obm.Insert_Date > r.initialErrorDate;  -- After the initial error (matching fix attempt summary logic)
