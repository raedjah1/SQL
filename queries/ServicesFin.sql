...
INNER JOIN Plus.pls.PartLocation pl_to ON pl_to.LocationNo = pt.ToLocation
    ...
    AND UPPER(LTRIM(RTRIM(pl_to.Warehouse))) = 'SERVICESFINGOODS'
...
-- Get parent serial for Family, Product Type, and LOB
LEFT JOIN Plus.pls.PartSerial ps_parent ON ps_parent.SerialNo = cleaned_part.CleanParentSN
-- Family from PARENT Serial (optimized lookup - gets latest value)
OUTER APPLY (
...
-- Product Type from PARENT Serial's PartNo
LEFT JOIN Plus.pls.PartNo pn_parent ON pn_parent.PartNo = REPLACE(ps_parent.PartNo, '-H', '')
LEFT JOIN Plus.pls.CodePartType cpt_type_parent ON cpt_type_parent.ID = pn_parent.PartTypeID
-- LOB from PARENT Serial (optimized lookup - gets latest value)
OUTER APPLY (
...
OUTER APPLY (
    ...
    WHERE cgt.GenericTableDefinitionID = 258  -- TEARDOWN_DEMANDLIST
        AND cgt.C01 = cleaned_part.CleanPartNo
