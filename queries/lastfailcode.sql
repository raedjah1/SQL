...
-- Detailed Fail Parts Query - Individual records matching FailPareots summary logic
SELECT 
    -- Source columns
    d.SerialNumber,
    d.PartNumber,
...
WHERE d.Program = 'DELL_MEM'
  ...
  -- ✅ Only get the LAST test per serial number (most recent test)
  AND seq.Touch_Number = seq.Total_Tests_Ever  -- Last touch = highest touch number
  AND (
