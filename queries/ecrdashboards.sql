...
FROM (
    SELECT 
        ...
        -- First check: If total hours < 24, return 0 days (regardless of business hours)
        ...
        CASE 
            WHEN (
                ...
                END
            ) < 24.0 THEN 0  -- Less than 24 total hours = 0 days (e.g., 9:28 AM yesterday to 9:25 AM today)
            ELSE (
                CASE 
                    WHEN MAX(pt.CreateDate) IS NOT NULL THEN DATEDIFF(DAY, SOH.CreateDate, MAX(pt.CreateDate))
                    WHEN SOL.StatusID = 18 THEN DATEDIFF(DAY, SOH.CreateDate, SOH.LastActivityDate)
