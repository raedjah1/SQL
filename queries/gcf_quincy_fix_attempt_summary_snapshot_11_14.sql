-- ============================================================================
-- FIX ATTEMPT SUMMARY - SNAPSHOT VERSION (11/14/2025)
-- ============================================================================
-- This query matches Excel's snapshot behavior by only considering BizTalk
-- messages up to a specific snapshot date/time
-- ============================================================================
-- INSTRUCTIONS:
-- 1. Set @SnapshotDateTime to the date/time Excel was created
-- 2. Common dates to try:
--    - '2025-11-19 23:59:59' (end of 11/19)
--    - '2025-11-20 00:00:00' (start of 11/20)
--    - '2025-11-20 11:00:00' (before most errors appeared)
-- ============================================================================

DECLARE @SnapshotDateTime DATETIME = '2025-11-19 23:59:59';  -- ADJUST THIS DATE/TIME TO MATCH EXCEL
...
FROM (
    SELECT 
        ...
        -- Get latest error message from BizTalk (if exists, up to snapshot time)
        latestError.LatestErrorMessage,
        -- Determine error status: "Encountered New Error", "Same Error", or "Resolved"
        -- SNAPSHOT VERSION: Only considers BizTalk messages up to @SnapshotDateTime
        CASE 
            ...
            -- SNAPSHOT: Only check errors up to @SnapshotDateTime
            WHEN EXISTS (
                SELECT 1
                ...
                WHERE obm.Source = 'Plus'
                  ...
                  AND obm.Insert_Date <= @SnapshotDateTime  -- SNAPSHOT: Only up to snapshot time
    ...
    OUTER APPLY (
        ...
        -- SNAPSHOT: Only get errors up to @SnapshotDateTime
        SELECT TOP 1
            LTRIM(SUBSTRING(
...
-- ============================================================================
-- SUMMARY: Show total counts to compare with Excel
-- ============================================================================
SELECT 
    'Snapshot DateTime Used' AS [Metric],
    CAST(@SnapshotDateTime AS VARCHAR(50)) AS [Value]
UNION ALL
SELECT 
    'Total Fix Attempts' AS [Metric],
        ...
        CASE 
            ...
            WHEN EXISTS (
                ...
                WHERE obm.Source = 'Plus'
                  ...
                  AND obm.Insert_Date <= @SnapshotDateTime
